...
//#endif
	...
	public bool TryPlayFX(RaycastHit hit)
	{
        ...
        //TODO AUDIO
        bool mainAudioSourceTaken = false;// Audio.isPlaying;
		vp_SurfaceManager.SpawnEffect(hit, ImpactEvent);
		...
		if (!m_HaveObjectSounds)
		{
			if (m_HaveSurfaceManager && mainAudioSourceTaken)
			{
                ...
                //TODO AUDIO
                if (ImpactEvent == null)
                    vp_SurfaceManager.SpawnEffect(hit, vp_SurfaceManager.Instance.Fallbacks.ImpactEvent, (GameObject)null);
				else
					vp_SurfaceManager.SpawnEffect(hit, ImpactEvent, (GameObject)null);
			}
			return false;
		}
	...
	public bool TryPlaySound(vp_SurfaceType surfaceType)
	{
		...
		for (int v = CollisionSounds.Count - 1; v > -1; v--)
		{
			...
			{
				//DoPlaySound(CollisionSounds[v].Sound.Post(gameObject);

                //play Collision Wwise event
                CollisionSounds[v].Sound.Post(gameObject);
                playedSound = true;
			}
        ...
        {
            //DoPlaySound(CollisionSounds[fallback].Sound.Post(gameObject);

            //play Collision Wwise event backup
            CollisionSounds[fallback].Sound.Post(gameObject);
        }
	...
	/// </summary>
	void DoPlaySound(AK.Wwise.Event sound)
	{
