...
public class vp_Grab : vp_Interactable
{
	...
	protected virtual void StartGrab()
	{
        ...
        // play a grab sound
        if (GrabSound != string.Empty)
        {
            //FMODUnity.RuntimeManager.PlayOneShotAttached(GrabSound, gameObject);
        }
	...
	protected virtual void StopGrab()
	{
		...
		if (FPPlayer.InputGetButtonDown.Send("Attack"))
		{
            if (ThrowSound != string.Empty)
            {
                //FMODUnity.RuntimeManager.PlayOneShotAttached(ThrowSound, gameObject);
            }
            if (Rigidbody != null)
			{
		...
		{
            if (DropSound != string.Empty)
            {
                //FMODUnity.RuntimeManager.PlayOneShotAttached(DropSound, gameObject);
            }
