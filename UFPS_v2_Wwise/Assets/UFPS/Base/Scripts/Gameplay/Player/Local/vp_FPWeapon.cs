...
public class vp_FPWeapon : vp_Weapon
{
    ...
    // sound for bringing out the weapon
    public AK.Wwise.Event wieldSound;
    
    // sound for putting the weapon away
    public AK.Wwise.Event unwieldSound;
	...
	public override void Wield(bool isWielding = true)
	{
        ...
        // play sound
        //if ((isWielding ? SoundWield : SoundUnWield) != string.Empty)
        //{
        	if (vp_Utility.IsActive(gameObject))
        	{

            rootParent = gameObject.transform.root.gameObject;

            if (isWielding == true)
                {
                    wieldSound.Post(rootParent);
                }
                else if (isWielding == false)
                {
                    unwieldSound.Post(rootParent);
                }
            }
        //}
